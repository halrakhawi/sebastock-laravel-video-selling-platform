...
class UserController extends Controller
{
    ...
    public function home($lang){
        $adv1=DB::table('advertisments')->where('type','=',2)->where('status','=',1)->inRandomOrder()->first();
        $adv2=DB::table('advertisments')->where('type','=',2)->where('status','=',1)->inRandomOrder()->first();
        session()->put(['adv1'=> $adv1,'adv2'=> $adv2]);
        //session()->put('adv2', $adv2);
        session()->put('locale', $lang);
        APP::setLocale($lang);
        if (auth('seller')->user()||auth('admin')->user()||auth('user')->user()) {
        return redirect()->route('front.indexlogining',App()->getLocale());
        session()->remove('adv1', $adv1);
        session()->remove('adv2', $adv2);
    }
    else 
