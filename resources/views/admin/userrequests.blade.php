...
<li class="breadcrumb-item active">{{__('User Requests')}}</li>
</ul>
</div>
{{-- <div class="col-sm-12 col-md-6 col-lg-6 right">               
<button class="backbtn" style="margin-top: 10px; "><i class="fas fa-arrow-down" style="color: #ffffff !important;"></i><a href="{{route('admin.backup',App()->getLocale())}}" class="abackup"> Backup Data</a></button>
</div> --}}
</div>
</div>
...
<div class="">
  <div class="card-close" style="position: absolute; display: flex;">
    <div class="dropdown itemstop">
      ...
      <div aria-labelledby="closeCard1" class="dropdown-menu dropdown-menu-right has-shadow">
       <a href="{{route('admin.userrequestsort',['sort-asc',App()->getLocale()])}}" class="dropdown-item ATZ">
        {{__('A TO Z')}}</a>
        <a href="{{route('admin.userrequestsort',['sort-desc',App()->getLocale()])}}" class="dropdown-item ZTA">
          {{__('Z TO A')}}</a>
      </div>
      </div>
      <div class="dropdown itemstop">
      ...
      <div aria-labelledby="closeCard2" class="dropdown-menu dropdown-menu-right has-shadow ">
       <a href="{{route('admin.userrequestsort',['sort-recent',App()->getLocale()])}}" class="dropdown-item">
        {{__('Recently')}} </a>
        <a href="{{route('admin.userrequestsort',['sort-old',App()->getLocale()])}}" class="dropdown-item ">
          {{__('Oldest')}} </a>
  ...
  <div class="card-header d-flex align-items-center">
    <h3 class="h4">{{__('User Requests')}}</h3>
  </div>
  <div class="card-body">
    <div class="table-responsive">
      <table class="table">
        ...
        <tbody>
@foreach ($Adminbuyers as $item)
@if($item->accept_reject_request==0)
          <tr>
...
@foreach ($Adminsellers as $item)
@if($item->accept_reject==0)
          <tr>
            ...
            <td>{{$item->created_at}}</td>
            <td>{{isSeller('seller_id')}}</td>
            <td>                         
              <div class="card-close tddivdd"> 
                <button type="submit>" class="dropdown-item remove acceptdiv"><a href="{{route('admin.sellerDetails',$item->seller_id)}}" class="dropdown-item "><i class="fas fa-eye"></i></a></button>
                    {{-- <div class="dropdown">
                      <button type="button" id="closeCard1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-toggle morebutton"><i class="fa fa-ellipsis-v"></i></button>
                      <div aria-labelledby="closeCard1" class="dropdown-menu dropdown-menu-right has-shadow">
                          <button type="submit>" class="dropdown-item remove acceptdiv"><a href="{{route('admin.sellerActivate',$item->seller_id)}}"><i class="fas fa-check"></i>Accepts</a></button>
                          <a href="#" class="dropdown-item edit rejectdiv"><i class="fas fa-ban"></i>Rejection</a>
                          <a href="#" class="dropdown-item edit actionsubadmin2"><i class="fas fa-trash-alt"></i>delete</a>
                        </div> 
                        </div>                                      --}}
