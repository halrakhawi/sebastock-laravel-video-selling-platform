...
@section('content')
  ...
  <div class="divmainFormlogin">
    ...
    <p class="plogin">{{__('Don't worry! Just fill you email and we'll send you a link to reset your password')}}</p>
    <div class="divformlogin">
        <form action="{{route('seller.postEmail')}}" method="post">
        <div>
        ...
        <div>
            <p class="psignup">{{__('Back to')} <a href="{{route('sellerlogin',App()->getLocale())}}"><span class="spansignup">{{__('Login')}}</span></a></p>
        </div>
    </div>
  </div>
